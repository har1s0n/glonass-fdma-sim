...
public:
   ...
   // Предусл. (В.9, поз.22): n0>=0; Fs>0; |Δf_k| + B_model <= Fs/2.
   // Тракт L1OC (В.2, поз.34): f_k = f0 = carrierFreqL1OC => Δf_j=0, Θ_j[n] ≡ Θ_j[0] (§1 (1.7));
   // полоса модели иная, поэтому подаётся modelBandwidth = modelBandwidthL1OC (2·f_T1).
   void init(SampleIndex  globalStartSample,          // n0 >= 0
             std::int64_t sampleRate,                 // Fs, Гц
             std::int64_t carrierFreq,                // f_k, Гц ([ИКД] 3.3.1.1); L1OC — f_L1OC ([ИКД-L1OC] 2.1.1)
             ...
             std::int64_t modelBandwidth = codeRate); // B_model, Гц: односторонняя полоса модели (В.2)
