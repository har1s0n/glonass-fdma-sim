...
public:
   ...
   std::size_t samplesWritten() const noexcept {
...
struct IqMetadata {
   ...
   std::string         band;               // "L1" | "L2" | "L1OC"
   std::int64_t        n0         = 0;     // globalStartSample
   std::size_t         numSamples = 0;     // фактически записано
   std::vector<int>    literals;           // K, по возрастанию (FDMA-тракт)
   std::vector<int>    satellites;         // J, по возрастанию (тракт L1OC, § 0.1 поз.28)
   std::vector<double> amplitudes;         // A_k / A_j, в порядке literals / satellites
   double              eta     = 1.0;      // η = 1/√(Σ A²)
...
inline void writeSidecarJson(const std::string& path, const IqMetadata& meta) {
   ...
   // Состав источников: literals — сетка литер FDMA, satellites — активное множество J тракта
   // L1OC. Печатается непустой массив: тракты не смешиваются, выход FDMA-модуля не меняется.
   if (!meta.literals.empty()) {
      out << "  \"literals\": "       << intArray(meta.literals)    << ",\n";
   }

   if (!meta.satellites.empty()) {
      out << "  \"satellites\": "     << intArray(meta.satellites)  << ",\n";
   }
   out << "  \"amplitudes\": "        << dblArray(meta.amplitudes)  << ",\n"
       << "  \"eta\": "               << etaBuf               << ",\n"
